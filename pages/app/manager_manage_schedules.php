...
if(isset($_GET['delete'])){ $id=(int)$_GET['delete']; $stmt=$conn->prepare('SELECT start_time FROM schedules WHERE schedule_id=? LIMIT 1'); $stmt->bind_param('i',$id); $stmt->execute(); $row=$stmt->get_result()->fetch_assoc(); if($row && strtotime($row['start_time']) < time()){ header('Location: '.$self_file.'?error='.urlencode('Cannot remove a past schedule slot')); exit; } $stmt=$conn->prepare('DELETE FROM schedules WHERE schedule_id=?'); $stmt->bind_param('i',$id); $stmt->execute(); header('Location: '.$self_file.'?success='.urlencode('Schedule removed')); exit; }
...
$week_start = monday_of_week($_GET['week_start'] ?? $_POST['week_start'] ?? date('Y-m-d'));
if($_SERVER['REQUEST_METHOD']==='POST' && isset($_POST['save_slots'])){
    ...
    foreach($slots as $slot){
        ...
        if(strtotime($start) < time()){ $skipped++; continue; }
    ...
    header('Location: '.$self_file.'?resource_type='.urlencode($resource_type).'&resource_id='.$resource_id.'&week_start='.$week_start.'&success='.urlencode($msg)); exit;
}
...
$upcoming=[];
if($resource_id>0){
    if($resource_type==='room') $stmt=$conn->prepare("(SELECT 'Booking' kind, booking_start start_time, booking_end end_time, booking_status status, purpose notes FROM bookings WHERE resource_type='room' AND room_id=? AND booking_end>=NOW()) UNION ALL (SELECT 'Schedule' kind, start_time, end_time, status, notes FROM schedules WHERE resource_type='room' AND room_id=? AND end_time>=NOW()) ORDER BY start_time LIMIT 12");
    else $stmt=$conn->prepare("(SELECT 'Booking' kind, booking_start start_time, booking_end end_time, booking_status status, purpose notes FROM bookings WHERE resource_type='facility' AND facility_id=? AND booking_end>=NOW()) UNION ALL (SELECT 'Schedule' kind, start_time, end_time, status, notes FROM schedules WHERE resource_type='facility' AND facility_id=? AND end_time>=NOW()) ORDER BY start_time LIMIT 12");
    $stmt->bind_param('ii',$resource_id,$resource_id); $stmt->execute(); $upcoming=$stmt->get_result()->fetch_all(MYSQLI_ASSOC);
}
...
<input class="input" type="date" name="week_start" min="<?= h(date('Y-m-d')) ?>" value="<?= h($week_start) ?>" onchange="this.form.submit()">
...
<input type="hidden" name="resource_type" value="<?= h($resource_type) ?>"><input type="hidden" name="resource_id" value="<?= h($resource_id) ?>"><input type="hidden" name="week_start" value="<?= h($week_start) ?>"><input type="hidden" name="save_slots" value="1"><div id="slotInputs"></div>
...
<div class="overflow-x-auto"><table class="border-collapse w-full"><thead><tr><th class="table-th sticky left-0 bg-slate-50">Weekday</th><?php foreach($hours as $h): ?><th class="table-th text-center"><?= sprintf('%02d.00-%02d.00',$h,$h+1) ?></th><?php endforeach; ?></tr></thead><tbody><?php foreach($weekdays as $i=>$day): $date=$table_dates[$i]; ?><tr><th class="table-th sticky left-0 bg-slate-50"><?= h($day) ?><p class="text-[10px] normal-case tracking-normal"><?= h($date) ?></p></th><?php foreach($hours as $h): $key=$date.'|'.$h; $slotStart=$date.' '.str_pad((string)$h,2,'0',STR_PAD_LEFT).':00:00'; $isPast=strtotime($slotStart)<time(); $cls='slot-free'; $text='Available'; $disabled=false; if($isPast){ $cls='slot-maintenance'; $text='Past'; $disabled=true; } elseif(isset($booked[$key])){ $cls='slot-booked'; $text='Booked<br><span class="text-[10px]">'.h($booked[$key]).'</span>'; $disabled=true; } elseif(isset($manual[$key])){ $cls='slot-'.$manual[$key]['status']; $text=ucfirst($manual[$key]['status']).'<br><a class="underline text-[10px]" onclick="event.stopPropagation();return confirm(\'Remove this schedule?\')" href="?delete='.h($manual[$key]['schedule_id']).'">remove</a>'; } ?><td class="slot-cell <?= $cls ?>" data-slot="<?= h($key) ?>" data-disabled="<?= $disabled?'1':'0' ?>"><?= $text ?></td><?php endforeach; ?></tr><?php endforeach; ?></tbody></table></div>
