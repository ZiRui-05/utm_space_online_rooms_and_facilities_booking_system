...
<header class="no-print flex justify-between items-center w-full px-10 py-4 sticky top-0 z-50 bg-white border-b border-slate-200">
    ...
    <div class="flex items-center gap-4">
        <div class="relative p-1.5 text-slate-500 hover:text-red-900 transition-colors cursor-pointer">
            <span class="material-symbols-outlined">notifications</span>
            <span class="absolute top-0 right-0 bg-amber-500 text-white font-black text-[10px] w-4 h-4 flex items-center justify-center rounded-full">2</span>
        </div>

        <a href="../../homepage.php" class="bg-amber-500 hover:bg-amber-600 text-slate-900 text-sm font-bold px-4 py-2 rounded-md transition-colors shadow-sm">
            Book Now
        </a>
        ...
        <div class="relative inline-block text-left">
            <button id="avatarDropdownBtn" class="h-9 w-9 rounded-full overflow-hidden border border-slate-200 focus:outline-none focus:ring-2 focus:ring-red-900 transition-all flex items-center justify-center cursor-pointer">
            ...
            <div id="avatarDropdownMenu" class="hidden absolute right-0 mt-2 w-56 rounded-xl bg-white shadow-xl border border-slate-100 py-1.5 z-50 origin-top-right transition-all">
                ...
                <a href="<?= h($dashboardLink) ?>" class="flex items-center gap-3 px-4 py-2.5 text-sm text-slate-700 hover:bg-slate-50 transition-colors">
                    <span class="material-symbols-outlined text-blue-500 text-lg">shield</span>
                ...
                <a href="<?php echo $isAdmin ? 'admin_booking_requests.php' : 'manager_booking_requests.php'; ?>" class="flex items-center gap-3 px-4 py-2.5 text-sm text-slate-700 hover:bg-slate-50 transition-colors">
                    <span class="material-symbols-outlined text-orange-400 text-lg">description</span>
                ...
                <a href="about.php" class="flex items-center gap-3 px-4 py-2.5 text-sm text-slate-700 hover:bg-slate-50 transition-colors">
                    <span class="material-symbols-outlined text-pink-500 text-lg">help</span>
                    <span>About</span>
                </a>
                ...
                <a href="logout.php" class="flex items-center gap-3 px-4 py-2.5 text-sm text-slate-700 hover:bg-slate-50 transition-colors">
                    <span class="material-symbols-outlined text-amber-700 text-lg">logout</span>
                    <span>Logout</span>
                </a>
            </div>
        </div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const menuEl = document.getElementById('avatarDropdownMenu');

    if (avatarBtn && menuEl) {
        // Toggle absolute display class via event hook listener click callback
        avatarBtn.addEventListener('click', function (e) {
            e.stopPropagation();
            menuEl.classList.toggle('hidden');
        });

        // Global interface overlay capture handler to automatically auto-dismiss open menus 
        document.addEventListener('click', function (e) {
            if (!avatarBtn.contains(e.target) && !menuEl.contains(e.target)) {
