...
<head>
    ...
    <style>body{font-family:Inter,sans-serif;background:#f8f9ff}.material-symbols-outlined{font-variation-settings:'FILL' 0,'wght' 400,'GRAD' 0,'opsz' 24;vertical-align:middle}.table-th{padding:14px 18px;font-size:11px;text-transform:uppercase;letter-spacing:.08em;color:#64748b;background:#f8fafc;text-align:left}.table-td{padding:14px 18px;border-top:1px solid #e2e8f0;font-size:14px;vertical-align:top}.input{width:100%;border:1px solid #dcc0c2;border-radius:8px;padding:10px 12px;background:#fff}.btn-primary{background:#5c001f;color:#fff;padding:10px 16px;border-radius:8px;font-weight:700;display:inline-block}.btn-warning{background:#fdaa1d;color:#291800;padding:10px 16px;border-radius:8px;font-weight:700;display:inline-block}.btn-light{background:#f8fafc;border:1px solid #dcc0c2;padding:10px 16px;border-radius:8px;font-weight:700;display:inline-block}.badge{display:inline-flex;align-items:center;border-radius:999px;padding:4px 10px;font-size:12px;font-weight:700}.badge-pending{background:#fff7ed;color:#9a3412}.badge-approved,.badge-active,.badge-available,.badge-completed,.badge-paid,.badge-verified{background:#dcfce7;color:#166534}.badge-rejected,.badge-suspended,.badge-unavailable,.badge-cancelled,.badge-expired,.badge-unpaid,.badge-unverified,.badge-payment_rejected{background:#fee2e2;color:#991b1b}.badge-maintenance,.badge-inactive,.badge-blocked,.badge-pending_verification,.badge-refunded{background:#e2e8f0;color:#334155}.slot-cell{min-width:110px;height:58px;border:1px solid #e2e8f0;text-align:center;font-size:12px;cursor:pointer}.slot-free{background:#fff}.slot-selected{background:#fef3c7!important;outline:2px solid #f59e0b}.slot-booked{background:#dbeafe;color:#1e40af;cursor:not-allowed}.slot-blocked{background:#fee2e2;color:#991b1b}.slot-maintenance{background:#e2e8f0;color:#334155}@media print{.no-print,header,aside{display:none!important}main{margin-left:0!important;padding:0!important}.print-card{box-shadow:none!important;border-color:#111!important}.page-break{page-break-before:always}body{background:#fff!important;color:#111!important}}</style>
</head>
...
<aside class="no-print flex flex-col fixed left-0 top-0 pt-20 pb-8 px-4 z-40 bg-slate-50 h-screen w-64 border-r border-slate-200">
    ...
    <div class="flex-1 space-y-1">
        ...
        <?php if($isAdmin): ?>
            ...
            <a class="<?= nav_class('schedules', $active_page) ?>" href="admin_manage_schedules.php"><span class="material-symbols-outlined">calendar_month</span><span>Schedules</span></a>
        ...
        <?php else: ?>
            ...
            <a class="<?= nav_class('users', $active_page) ?>" href="manager_manage_users.php"><span class="material-symbols-outlined">group</span><span>Manager Accounts</span></a>
        <?php endif; ?>
    </div>
</aside>
