...
$next = $conn->query("SELECT b.*, u.full_name, COALESCE(r.room_name, f.facility_name) resource_name FROM bookings b JOIN users u ON u.user_id=b.user_id LEFT JOIN rooms r ON r.room_id=b.room_id LEFT JOIN facilities f ON f.facility_id=b.facility_id WHERE b.booking_status IN ('pending','approved') ORDER BY b.booking_start ASC LIMIT 8");
include __DIR__ . '/includes/header.php';
?>
<div class="mb-8"><h1 class="text-4xl font-black text-[#36000f]">Facility Manager Dashboard</h1><p class="text-slate-500 mt-2">Daily operations for rooms, facilities, schedules and booking approval.</p></div>
