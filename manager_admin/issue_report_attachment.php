...
$stmt = $conn->prepare('SELECT issue_id, reported_by, issue_hidden, attachment_name, attachment_mime, attachment_base64 FROM issue_reports WHERE issue_id = ? LIMIT 1');
$stmt->bind_param('i', $issueId);
$stmt->execute();
$report = $stmt->get_result()->fetch_assoc();

if (!$report || empty($report['attachment_base64']) || (($user['role'] ?? '') !== 'admin' && (int)($report['issue_hidden'] ?? 0) === 1)) {
    http_response_code(404);
    exit('Attachment not found.');
}
