...
if (isset($_GET['delete'])) { $id=(int)$_GET['delete']; $stmt=$conn->prepare('DELETE FROM facilities WHERE facility_id=?'); $stmt->bind_param('i',$id); $stmt->execute(); header('Location: admin_manage_facilities.php?success=Facility deleted'); exit; }
...
if($_SERVER['REQUEST_METHOD']==='POST'){
    ...
    header('Location: admin_manage_facilities.php?success='.urlencode($msg)); exit;
}
...
$facilities=$conn->query($sql); $page_title='Admin Manage Facilities'; $active_page='facilities'; include __DIR__ . '/includes/header.php';
?>
<div class="mb-8"><h1 class="text-4xl font-black text-[#36000f]">Admin - Facility Management</h1><p class="text-slate-500 mt-2">Search facilities, filter availability and manage facility records.</p></div>
