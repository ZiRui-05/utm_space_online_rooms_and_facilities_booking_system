...
if($_SERVER['REQUEST_METHOD']==='POST'){
    ...
    if(($_POST['action']??'')==='delete'){
        ...
        if($id>0 && $id!==(int)$user['user_id']){ $stmt=$conn->prepare('DELETE FROM users WHERE user_id=?'); $stmt->bind_param('i',$id); $stmt->execute(); }
